...
#
# supramark_plantuml_jni.c marshals a Java String <-> the C FFI. That C
# layer is plain JNI (only <android/log.h> is Android-specific, stubbed
# here), so it can be exercised on a host JVM without an Android NDK,
# emulator, gradle or React Native. This script:
#
#   1. builds the supramark-plantuml-native static lib for the host arch,
#   2. compiles supramark_plantuml_jni.c into a host JNI shared library
...
#   crates/plantuml-little/packages/react-native/scripts/test-android-jni.sh
set -euo pipefail
...
OUT="$(mktemp -d)"
case "$(uname)" in
  Darwin) SO="$OUT/libsupramark_plantuml_jni.dylib"; SHARED=(-dynamiclib); OSLIBS=(-framework CoreFoundation -framework Security);;
  *)      SO="$OUT/libsupramark_plantuml_jni.so";    SHARED=(-shared);      OSLIBS=(-lm -lpthread -ldl);;
esac
clang "${SHARED[@]}" -fPIC -O0 -g \
  -I"$JAVA_HOME/include" -I"$JAVA_HOME/include/darwin" -I"$JAVA_HOME/include/linux" \
  -I"$NATIVE_INCLUDE" -I"$STUBS" \
  "$JNI_DIR/supramark_plantuml_jni.c" \
  "$LIB" "${OSLIBS[@]}" \
  -o "$SO"
...
CLASSES="$OUT/classes"
mkdir -p "$CLASSES"
"$JAVA_HOME/bin/javac" -d "$CLASSES" "$TEST_DIR/SupramarkPlantumlModule.java"
exec "$JAVA_HOME/bin/java" --enable-native-access=ALL-UNNAMED \
  -cp "$CLASSES" com.supramark.plantumlnative.SupramarkPlantumlModule "$SO"

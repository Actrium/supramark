/*
 * SupramarkPlantumlModule.java — TEST-ONLY host-JVM harness for the JNI bridge.
 *
 ...
 * com.supramark.plantumlnative.SupramarkPlantumlModule, because JNI binds
 * native methods by mangled class name: the C symbols in
 * supramark_plantuml_jni.c are
 * Java_com_supramark_plantumlnative_SupramarkPlantumlModule_native*.
 *
...
public final class SupramarkPlantumlModule {
    // Status codes mirror the C ABI (SUPRAMARK_PLANTUML_*).
    ...
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println("usage: SupramarkPlantumlModule <path-to-jni-shared-lib>");
            System.exit(2);
        }
        System.load(args[0]);

        // 1. A real PlantUML diagram renders to an SVG document, status OK.
        int[] st = { -1 };
        String svg = nativeRender("@startuml\nAlice -> Bob: hi\n@enduml", st);
        ...
        //    empty string is a zero-length (not NULL) PlantUML source. It must

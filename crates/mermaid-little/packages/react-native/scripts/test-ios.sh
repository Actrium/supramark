...
#
# The SupramarkMermaidModule bridge marshals NSString <-> the C FFI; that
...
# Usage: crates/mermaid-little/packages/react-native/scripts/test-ios.sh
set -euo pipefail
...
OUT="$(mktemp -d)"
BUNDLE="$OUT/SupramarkMermaidModuleTests.xctest"
mkdir -p "$BUNDLE/Contents/MacOS"
BIN="$BUNDLE/Contents/MacOS/SupramarkMermaidModuleTests"
...
clang++ -g -O0 -fobjc-arc -std=c++17 \
  ...
  -bundle \
  "$IOS_DIR/SupramarkMermaidModule.mm" \
  "$TEST_DIR/SupramarkMermaidModuleTests.mm" \
  "$LIB" \
...
<dict>
  ...
  <key>CFBundleExecutable</key><string>SupramarkMermaidModuleTests</string>
  <key>CFBundleIdentifier</key><string>com.supramark.mermaidnative.tests</string>
  <key>CFBundleInfoDictionaryVersion</key><string>6.0</string>
  <key>CFBundleName</key><string>SupramarkMermaidModuleTests</string>

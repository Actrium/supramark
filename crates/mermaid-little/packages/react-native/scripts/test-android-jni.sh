...
#
# supramark_mermaid_jni.c marshals a Java String <-> the C FFI. That C layer
...
#
#   1. builds the supramark-mermaid-native static lib for the host arch,
#   2. compiles supramark_mermaid_jni.c into a host JNI shared library
...
#   crates/mermaid-little/packages/react-native/scripts/test-android-jni.sh
set -euo pipefail
...
NATIVE_MANIFEST="$REPO/crates/mermaid-little/packages/native/Cargo.toml"
NATIVE_INCLUDE="$REPO/crates/mermaid-little/packages/native/include"
...
LIB="$REPO/target/debug/libsupramark_mermaid_native.a"
[ -f "$LIB" ] || { echo "static lib not found at $LIB" >&2; exit 1; }

echo "==> [2/3] Compiling JNI shared library"
OUT="$(mktemp -d)"
case "$(uname)" in
  Darwin) SO="$OUT/libsupramark_mermaid_jni.dylib"; SHARED=(-dynamiclib); OSLIBS=(-framework CoreFoundation -framework Security);;
  *)      SO="$OUT/libsupramark_mermaid_jni.so";    SHARED=(-shared);      OSLIBS=(-lm -lpthread -ldl);;
esac
clang "${SHARED[@]}" -fPIC -O0 -g \
  -I"$JAVA_HOME/include" -I"$JAVA_HOME/include/darwin" -I"$JAVA_HOME/include/linux" \
  -I"$NATIVE_INCLUDE" -I"$STUBS" \
  "$JNI_DIR/supramark_mermaid_jni.c" \
  "$LIB" "${OSLIBS[@]}" \
  -o "$SO"
...
CLASSES="$OUT/classes"
mkdir -p "$CLASSES"
"$JAVA_HOME/bin/javac" -d "$CLASSES" "$TEST_DIR/SupramarkMermaidModule.java"
exec "$JAVA_HOME/bin/java" --enable-native-access=ALL-UNNAMED \
  -cp "$CLASSES" com.supramark.mermaidnative.SupramarkMermaidModule "$SO"

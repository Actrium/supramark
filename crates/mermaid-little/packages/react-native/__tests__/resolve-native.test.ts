...
const bridgeNative = {
  ...
  getVersion: async () => 'mermaid-bridge-1.0',
};
mock.module('react-native', () => ({
  NativeModules: { SupramarkMermaidNative: bridgeNative },
  Platform: {
...
describe('engine registration + delegation', () => {
  it('registers exactly one adapter for the "mermaid" engine', () => {
    expect(registered).toHaveLength(1);
    expect(registered[0].engine).toBe('mermaid');
  ...
  it('the registered adapter.render delegates to the resolved native render', async () => {
    const out = await registered[0].render('graph TD; A-->B');
    expect(out).toBe('<svg>graph TD; A-->B</svg>');
    expect(bridgeCalls).toContain('render:graph TD; A-->B');
  });

  it('getNativeVersion delegates to the resolved native getVersion', async () => {
    expect(await getNativeVersion()).toBe('mermaid-bridge-1.0');
  });
});

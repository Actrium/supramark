/*
 * SupramarkD2Module.java — TEST-ONLY host-JVM harness for the JNI bridge.
 *
 ...
 * com.supramark.d2native.SupramarkD2Module, because JNI binds native
 * methods by mangled class name: the C symbols in supramark_d2_jni.c are
 * Java_com_supramark_d2native_SupramarkD2Module_native*.
 *
...
public final class SupramarkD2Module {
    // Status codes mirror the C ABI (SUPRAMARK_D2_*).
    ...
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println("usage: SupramarkD2Module <path-to-jni-shared-lib>");
            System.exit(2);
        }
        System.load(args[0]);

        // 1. A real D2 diagram renders to an SVG document, status OK.
        int[] st = { -1 };
        String svg = nativeRender("a -> b", st);
        ...
        //    empty string is a zero-length (not NULL) D2 source. It must not
        //    crash and must report a definite status (OK with an SVG, or a
        //    parse/render error) — never silently corrupt the out-param.
        st[0] = -1;

...
#
# The SupramarkD2Module bridge marshals NSString <-> the C FFI; that
...
# Usage: crates/d2-little/packages/react-native/scripts/test-ios.sh
set -euo pipefail
...
OUT="$(mktemp -d)"
BUNDLE="$OUT/SupramarkD2ModuleTests.xctest"
mkdir -p "$BUNDLE/Contents/MacOS"
BIN="$BUNDLE/Contents/MacOS/SupramarkD2ModuleTests"
...
clang++ -g -O0 -fobjc-arc -std=c++17 \
  ...
  -bundle \
  "$IOS_DIR/SupramarkD2Module.mm" \
  "$TEST_DIR/SupramarkD2ModuleTests.mm" \
  "$LIB" \
...
<dict>
  ...
  <key>CFBundleExecutable</key><string>SupramarkD2ModuleTests</string>
  <key>CFBundleIdentifier</key><string>com.supramark.d2native.tests</string>
  <key>CFBundleInfoDictionaryVersion</key><string>6.0</string>
  <key>CFBundleName</key><string>SupramarkD2ModuleTests</string>
